...
LauncherBase {
    ...
    bottomPanelContent: RowLayout {
        ...
        Layout.minimumWidth: profilesettingsbox.leftMargin
                             + profilesettingsbox.implicitWidth + pbutton.minimumWidth
            ...
            RowLayout {
                ...
                ProfileComboBox {
                    ...
                    Component.onCompleted: {
                        ...
                        launcherSettingsWindow.currentGameDataDir = Qt.binding(
                                    function () {
                                        return (profileManager.activeProfile
                                                && profileManager.activeProfile.dataDirCustom) ? QmlUrlUtils.localFileToUrl(profileManager.activeProfile.dataDir) : ""
                                    })
                        loaded = true
                    }
                    ...
                    enabled: !(playDownloadTask.active
                               || apkExtractionTask.active
                               || gameLauncher.running)
                }
                ...
                MButton {
                    ...
                    onClicked: {
                        profileEditWindow.setProfile(
                                    profileComboBox.getProfile())
                        profileEditWindow.show()
                    }
                }
            }
        ...
        PlayButton {
            id: pbutton
            Layout.alignment: Qt.AlignHCenter
            text: (isVersionsInitialized
                   && playVerChannel.licenseStatus > 1 /* Fail or Succeeded */
                   ) ? ((googleLoginHelper.account !== null && playVerChannel.hasVerifiedLicense
                         || !LAUNCHER_ENABLE_GOOGLE_PLAY_LICENCE_CHECK) ? (gameLauncher.running ? qsTr("Open log") : (checkSupport() ? (needsDownload() ? (googleLoginHelper.account !== null ? (profileManager.activeProfile.versionType === ProfileInfo.LATEST_GOOGLE_PLAY && googleLoginHelper.hideLatest ? qsTr("Please sign in again") : qsTr("Download and play")) : qsTr("Sign in")) : qsTr("Play")) : qsTr("Unsupported Version"))).toUpperCase() : qsTr("Ask Google Again")) : qsTr(
                           "Please wait...")
            subText: (isVersionsInitialized
                      && (googleLoginHelper.account == null
                          || playVerChannel.licenseStatus > 1 /* Fail or Succeeded */
                          )) ? ((googleLoginHelper.account !== null
                                 && playVerChannel.hasVerifiedLicense
                                 || !LAUNCHER_ENABLE_GOOGLE_PLAY_LICENCE_CHECK) ? (gameLauncher.running ? qsTr("Game is running") : (getDisplayedVersionName() ? ("Minecraft " + getDisplayedVersionName()).toUpperCase() : qsTr("Please wait..."))) : "Failed to obtain apk url") : "..."
            Layout.fillWidth: true
            Layout.preferredHeight: 70
            Layout.minimumHeight: implicitHeight
            enabled: (isVersionsInitialized
                      && playVerChannel.licenseStatus > 1 /* Fail or Succeeded */
                      ) && !(playDownloadTask.active || apkExtractionTask.active
                             || updateChecker.active || !checkSupport())
                     && (gameLauncher.running || getDisplayedVersionName())
                     && (googleLoginHelper.account !== null
            ...
            onClicked: {
                ...
                } else if (googleLoginHelper.account !== null
                           && !playVerChannel.hasVerifiedLicense
                           && LAUNCHER_ENABLE_GOOGLE_PLAY_LICENCE_CHECK) {
    ...
    ApkExtractionTask {
        ...
        onError: function (err) {
            playDownloadError.text = qsTr(
                        "Error while extracting the downloaded file(s), <a href=\"https://github.com/minecraft-linux/mcpelauncher-ui-manifest/issues\">please report this error</a>: %1").arg(
                        err)
            playDownloadError.open()
        }
        onFinished: function () {
    ...
    function launcherLatestVersion() {
        ...
        for (var i = 0; i < versionManager.archivalVersions.versions.length; i++) {
            ...
            if (playVerChannel.latestVersionIsBeta
                    && launcherSettings.showBetaVersions || !ver.isBeta) {
                for (var j = 0; j < abis.length; j++) {
                    if (ver.abi === abis[j]) {
                        console.log("launcherLatestVersion: " + JSON.stringify(
                                        ver))
                        return ver
                    }
                }
    ...
    function needsDownload() {
        ...
        if (profile.versionType == ProfileInfo.LATEST_GOOGLE_PLAY)
            return !versionManager.versions.contains(
                        launcherLatestVersionscode())
        if (profile.versionType == ProfileInfo.LOCKED_CODE) {
            var dver = versionManager.versions.get(profile.versionCode)
            return !dver || !launcherSettings.showUnsupported
                    && !versionManager.checkSupport(dver)
        }
    ...
    function getDisplayedNameForCode(code) {
        ...
        if (archiveInfo !== null && (ver === null || ver.archs.length == 1
                                     && ver.archs[0] == archiveInfo.abi)) {
            return archiveInfo.versionName + " (" + archiveInfo.abi
                    + ((archiveInfo.isBeta ? ", beta" : "") + ")")
        }
        if (code === rowLayout.playVerChannel.latestVersionCode)
            return rowLayout.playVerChannel.latestVersion
                    + (playVerChannel.latestVersionIsBeta ? " (beta)" : "")
        if (ver !== null) {
            var profile = profileManager.activeProfile
            return qsTr("%1  (%2, %3)").arg(ver.versionName).arg(code).arg(
                        profile.arch.length ? profile.arch : ver.archs.join(
                                                  ", "))
        }
    }
    ...
    function getDisplayedVersionName() {
        ...
        if (profile.versionType === ProfileInfo.LATEST_GOOGLE_PLAY)
            return getDisplayedNameForCode(launcherLatestVersionscode())
                    || ("Unknown (" + launcherLatestVersionscode() + ")")
        if (profile.versionType === ProfileInfo.LOCKED_CODE)
            return getDisplayedNameForCode(profile.versionCode)
                    || ((profile.versionDirName ? profile.versionDirName : "Unknown")
                        + " (" + profile.versionCode + ")")
        if (profile.versionType === ProfileInfo.LOCKED_NAME)
    ...
    function getCurrentGameDir(profile) {
        if (profile.versionType === ProfileInfo.LATEST_GOOGLE_PLAY) {
            return versionManager.getDirectoryFor(
                        versionManager.versions.get(
                            launcherLatestVersionscode()))
        }
        if (profile.versionType === ProfileInfo.LOCKED_CODE)
            return versionManager.getDirectoryFor(versionManager.versions.get(
                                                      profile.versionCode))
        if (profile.versionType === ProfileInfo.LOCKED_NAME)
    ...
    function checkLauncherLatestSupport() {
        ...
        return versionManager.archivalVersions.versions.length == 0
                || launcherSettings.showUnsupported
                || (launcherSettings.showUnverified || findArchivalVersion(
                        latestCode) != null || checkRollForward(latestCode))
    }
    ...
    function checkRollForward(code) {
        ...
        for (var i = 0; i < rollfwds.length; i++) {
            ...
            if (rollfwds[i].minVersionCode <= code
                    && code <= rollfwds[i].maxVersionCode) {
                return true
            }
        }
        return false
    ...
    function checkGooglePlayLatestSupport() {
        if (versionManager.archivalVersions.versions.length == 0) {
            ...
            rowLayout.warnMessage = qsTr(
                        "No mcpelauncher-versiondb loaded cannot check support")
            rowLayout.warnUrl = ""
            return true
        }
        if (launcherSettings.showUnsupported
                || versionManager.archivalVersions.versions.length === 0) {
        ...
        if (playVerChannel.latestVersionIsBeta
            ...
            rowLayout.warnMessage = qsTr(
                        "Latest Minecraft Version %1 is a beta version, which are hidden by default (Click here for more Information)").arg(
                        playVerChannel.latestVersion
                        + (playVerChannel.latestVersionIsBeta ? " (beta)" : ""))
            rowLayout.warnUrl
                    = "https://github.com/minecraft-linux/mcpelauncher-manifest/issues/797"
            return false
        }
        ...
        rowLayout.warnMessage = qsTr(
                    "Latest Minecraft Version %1 compatibility is Unknown, supporting new Minecraft Versions is a feature Request (Click here for more Information)").arg(
                    playVerChannel.latestVersion
                    + (playVerChannel.latestVersionIsBeta ? " (beta)" : ""))
        ...
        if (profile.versionType === ProfileInfo.LOCKED_CODE) {
            ...
            if (dver && dver.archs.length > 0
                ...
                var abis = googleLoginHelper.getAbis(
                            launcherSettings.showUnsupported)
                var ver = findArchivalVersion(profile.versionCode)
                if (ver !== null && (playVerChannel.latestVersionIsBeta
                                     && (launcherSettings.showBetaVersions
                                         || launcherSettings.showUnsupported)
                                     || !ver.isBeta)) {
                    for (var j = 0; j < abis.length; j++) {
                ...
                return launcherSettings.showUnverified
                        || launcherSettings.showUnsupported
            }
        }
        if (profile.versionType === ProfileInfo.LOCKED_NAME) {
            return launcherSettings.showUnsupported
                    ...
                    && versionManager.checkSupport(profile.versionDirName)
        }
        console.log("Failed")
        return false
    }
